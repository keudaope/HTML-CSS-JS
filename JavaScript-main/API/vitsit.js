...
async function test() {
    try {
        const response = await fetch('./vitseja.json');
        const data = await response.json();
        const maara = Object.keys(data).length;
        const sat = Math.floor(Math.random() * maara);
        //const num = 50;
        VoiceRSS.speech({
            key: "d8840a46e25545ce8387e705929b87c1",
            src: data[sat].setup,
            hl: 'fi-fi',
            v: 'Linda',
            r: 0,
        ...
        setTimeout(async function () {
            try {
                VoiceRSS.speech({
                    //key: "b8dcbecde7e44a4ca4a81e0c6af53478",
                    key: "d8840a46e25545ce8387e705929b87c1",
                    src: data[sat].punchline,
                    hl: 'fi-fi',
                    v: 'Linda',
                    r: 0,
                    c: 'mp3',
